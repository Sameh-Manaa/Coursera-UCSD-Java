...
public class EarthquakeCityMap extends PApplet {
    ...
    private void printQuakes() {
        ...
        Map<String, Integer> countryQuakesCount = new HashMap<>();

        for (Marker qm : quakeMarkers) {
            if (qm instanceof LandQuakeMarker) {
                String hKey = ((LandQuakeMarker) qm).getCountry();
                int hValue = (countryQuakesCount.containsKey(hKey)) ? countryQuakesCount.get(hKey) + 1 : 1;
                countryQuakesCount.put(hKey, hValue);
            } else {
                int hValue = (countryQuakesCount.containsKey("OCEAN QUAKES")) ? countryQuakesCount.get("OCEAN QUAKES") + 1 : 1;
                countryQuakesCount.put("OCEAN QUAKES", hValue);
            }
        }

        for (Map.Entry<String, Integer> entry : countryQuakesCount.entrySet()) {
